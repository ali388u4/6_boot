...
def build_subjects_kb(subjects) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for s in subjects:
        kb.button(text=s.name, callback_data=f"qb:subject:{s.id}")
    kb.adjust(1)
    return kb.as_markup()


def build_chapters_kb(chapters) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for c in chapters:
        kb.button(text=c.title, callback_data=f"qb:chapter:{c.id}")
    kb.adjust(1)
    return kb.as_markup()
